...
export default function AdminLayout({
  ...
  // جلوگیری از اسکرول صفحه اصلی وقتی منوی موبایل باز است
  useEffect(() => {
    if (isMobileMenuOpen) {
  ...
  // تابع تغییر زبان قدرتمند (ذخیره دائمی و جلوگیری از تداخل CSS)
  const toggleLanguage = () => {
    const newLocale = locale === 'fa' ? 'en' : 'fa';
    
    // ذخیره زبان در کوکی (جهت هماهنگی با Middleware سایت)
    ...
    // رفرش کامل برای تغییر قطعی استایل‌های RTL و LTR
    window.location.href = newPathname;
  };

  // ترجمه منوها
  const menuItems = [
    { title: isRtl ? 'داشبورد مدیریتی' : 'Dashboard', href: `/${locale}/admin`, icon: LayoutDashboard },
    { title: isRtl ? 'مدیریت کاربران' : 'Users', href: `/${locale}/admin/users`, icon: Users },
    { title: isRtl ? 'پنل CRM' : 'CRM & Sales', href: `/${locale}/admin/crm`, icon: Briefcase },
    { title: isRtl ? 'لجستیک و بارها' : 'Logistics', href: `/${locale}/admin/logistics`, icon: Package },
    { title: isRtl ? 'مدیریت محتوا' : 'CMS', href: `/${locale}/admin/cms`, icon: FileText },
    { title: isRtl ? 'تنظیمات نرخ‌ها' : 'Rate Settings', href: `/${locale}/admin/settings/rates`, icon: Settings },
  ];

  return (
    // مشخص کردن صریح dir برای جلوگیری از در هم ریختگی تب‌ها و جدول‌ها
    <div dir={isRtl ? 'rtl' : 'ltr'} className="flex min-h-screen bg-[#050505] text-white font-vazirmatn selection:bg-yellow-500/30">
      ...
      <aside className={cn(
        ...
        // منطق نمایش/مخفی شدن در موبایل (حل مشکل تداخل)
        ...
        <div className="h-20 lg:h-24 flex flex-col items-center justify-center border-b border-white/5 bg-[url('/hero/hero-bg.avif')] bg-cover bg-center relative shrink-0">
          ...
          <div className="relative z-10 flex items-center gap-3 w-full px-6">
             <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-500 to-yellow-700 flex items-center justify-center shadow-lg shadow-yellow-500/20 shrink-0">
                <ShieldCheck size={24} className="text-black" />
             </div>
             <div>
                <span className="block text-base lg:text-lg font-black text-white tracking-tight">SAM ADMIN</span>
        ...
        <nav className="flex-1 p-4 lg:p-6 space-y-1.5 overflow-y-auto">
          ...
          {menuItems.map((item) => (
            ...
            >
              <item.icon size={20} className={cn("relative z-10 transition-colors shrink-0", pathname === item.href ? "text-yellow-500" : "group-hover:text-yellow-500")} />
              <span className="relative z-10 font-medium text-sm truncate">{item.title}</span>
              
              {/* خط نشانگر صفحه فعال */}
              <div className={cn(
                 "absolute top-0 bottom-0 w-1 bg-yellow-500 transition-all duration-300",
      ...
      {/* 🖼 Main Content Area (استفاده از padding برای جلوگیری از تداخل با سایدبار فیکس شده) */}
      <div className={cn(
        ...
        <header className="h-20 bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/5 sticky top-0 z-40 px-4 lg:px-8 flex items-center justify-between shadow-sm shrink-0">
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <button 
              ...
              className="lg:hidden p-2 rounded-xl bg-zinc-900 border border-white/10 text-zinc-400 hover:text-white transition-colors shrink-0 focus:outline-none focus:ring-2 focus:ring-yellow-500/50"
            >
              <Menu size={24} />
            </button>
            ...
            <div className="relative hidden lg:block group">
              ...
              <input 
                ...
                className={cn(
                   "w-80 h-11 bg-zinc-900/50 border border-white/5 rounded-xl text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-yellow-500/50 focus:bg-zinc-800 transition-all",
          ...
          <div className="flex items-center gap-2 sm:gap-4 lg:gap-6">
            ...
            <button 
              ...
              title={isRtl ? 'تغییر زبان به انگلیسی' : 'Change Language to Persian'}
            >
              <Globe size={16} className="text-yellow-500 shrink-0" />
